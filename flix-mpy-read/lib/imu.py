...
def _read_byte(reg: int) -> int:
    return _i2c.readfrom_mem(MPU9250_ADDR, reg, 1)[0]

def _read_bytes(reg: int, length: int) -> bytes:
    return _i2c.readfrom_mem(MPU9250_ADDR, reg, length)

def _write_byte(reg: int, value: int) -> None:
