...
class LowPassFilter:
    ...
    """
    def __init__(self, alpha: float = 1.0, initial: Optional['Vector'] = None) -> None:
        self.alpha = alpha
        if initial is None:
            self.output = Vector()
        else:
    ...
    def update(self, input_val: Union['Vector', float]) -> 'Vector':
    ...
    def set_cutoff_frequency(self, cutoff_freq: float, dt: float) -> None:
        import math
...
class LowPassFilterScalar:
    def __init__(self, alpha: float = 1.0, initial: float = 0.0) -> None:
        self.alpha = alpha
        self.output = initial
